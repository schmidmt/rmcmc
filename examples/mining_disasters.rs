...
fn main() -> io::Result<()> {
    ...
    let sub_steppers: Vec<Box<dyn steppers::SteppingAlg<Model, _>>> = vec![
        Box::new(
            steppers::SRWM::new(early_mean_parameter, &log_likelihood, None)
                .unwrap(),
        ),
        Box::new(
            steppers::SRWM::new(late_mean_parameter, &log_likelihood, None)
                .unwrap(),
        ),
        Box::new(
            steppers::DiscreteSRWM::new(
    ...
    println!(
        ...
        diagnostics::rhat(
            results
                .iter()
                .map(|c| c.iter().map(|m| m.early_mean).collect::<Vec<f64>>())
                .collect()
        )
    );
    println!(
        ...
        diagnostics::rhat(
            results
                .iter()
                .map(|c| c.iter().map(|m| m.late_mean).collect::<Vec<f64>>())
                .collect()
        )
    );
    println!(
