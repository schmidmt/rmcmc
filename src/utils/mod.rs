...
pub use mean_and_variance::*;

// #[cfg(test)]
// pub mod geweke;

/// Repeat the given function `n_tries` times or until it returns true
...
/// An Rv which is a single Rv extended to multiple dimensions
#[derive(Clone, Debug)]
...
impl<X, T> ContinuousDistr<Vec<X>> for MultiRv<X, T> where T: Rv<X> + Support<X> {}

impl<X, T> DiscreteDistr<Vec<X>> for MultiRv<X, T> where T: Rv<X> + Support<X> {}
    ...
    X: Clone,
{
    fn mean(&self) -> Option<Vec<X>> {
        self.base
            .mean()
            .map(|m| (0..self.dims).map(|_| m.clone()).collect())
    }
}
